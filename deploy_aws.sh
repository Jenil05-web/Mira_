#!/bin/bash
# ══════════════════════════════════════════════════════════════════════════
# deploy_aws.sh — Deploy MIRA to AWS App Runner
# Run from Mira_project root: bash deploy_aws.sh
#
# Prerequisites:
#   1. AWS account created at https://aws.amazon.com
#   2. AWS CLI installed: https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html
#   3. Docker Desktop installed and running
#   4. Run once: aws configure  (enter your AWS Access Key + Secret)
# ══════════════════════════════════════════════════════════════════════════
...
store_secret() {
    local NAME=$1
    local VALUE=$2
    aws secretsmanager describe-secret --secret-id "mira/$NAME" \
        --region $AWS_REGION 2>/dev/null && \
